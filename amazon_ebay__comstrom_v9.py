...
def searchpriceway4(product_price,url,max_retries,retry_delay):
    product_price = None
    for retry in range(max_retries):
        response = requests.get(url, headers={'User-Agent': '', 'Accept-Language': 'en-US, en;q=0.5'})
        soup = BeautifulSoup(response.text, 'html.parser')
        try:
            whole_price = soup.select('span.a-button-inner')
            match = re.search(r'\$\d+\.\d+', str(whole_price))
            if match:
            ...
            pass
        if product_price != None and price != '' and price != '.':
            break
        else:
            print(f" price Attempt {retry + 1} failed. Retrying in {retry_delay} seconds...")
            time.sleep(retry_delay)
    
def searchpriceway3(product_price,url,max_retries,retry_delay):
    product_price = None
    for retry in range(max_retries):
        response = requests.get(url, headers={'User-Agent': '', 'Accept-Language': 'en-US, en;q=0.5'})
        soup = BeautifulSoup(response.text, 'html.parser')
        try:
            whole_price = soup.select('#item_package_quantity_0')
            match = re.search(r'\$\d+\.\d+', str(whole_price))
            if match:
                price_str = match.group()[1:] 
                price_str = price_str.replace(',', '') 
...
def searchpriceway2(product_price,url,max_retries,retry_delay):
    product_price = None
    for retry in range(max_retries):
        response = requests.get(url, headers={'User-Agent': '', 'Accept-Language': 'en-US, en;q=0.5'})
        soup = BeautifulSoup(response.text, 'html.parser')
...
def searchpriceway1(product_price,amazon_soup):
    try:
    ...
    if whole_price == '' and fraction_price != '' :
        ...
        product_price = re.sub(r'\.{2,}', '.', product_price)
        if product_price != '.' :
...
def get_product_price(amazon_soup,url, max_retries, retry_delay):
    price_extraction_functions = [
        searchpriceway1,
        searchpriceway2,
        searchpriceway3,
        searchpriceway4
    ...
    for extraction_function in price_extraction_functions:
        product_price = extraction_function(None, amazon_soup) if extraction_function == searchpriceway1 else extraction_function(None, url, max_retries, retry_delay)
        if is_valid_price(product_price):
            return product_price

    # If none of the methods return a valid price, return None
    return None
def is_valid_price(price):
    # Add any additional validation logic if needed
    return price is not None and price != '' and price != '.'
    ...
    if "https://a.co" in url or "amazon.c" in url:
        ...
        if choosen_data['Price']:
            try:
                ...
                product_price = get_product_price(amazon_soup,url, max_retries, retry_delay)
            data.append(product_price) 
        if choosen_data['Rate']:
            try:
